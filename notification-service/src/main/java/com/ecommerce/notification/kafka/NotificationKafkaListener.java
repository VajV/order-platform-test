...
public class NotificationKafkaListener {
    ...
    public void handleInventoryReserved(
        ...
        try {
            ...
            Map<String, String> variables = new HashMap<>();
            variables.put("orderId", event.getOrderId());
            variables.put("itemCount", String.valueOf(event.getQuantity()));
            variables.put("productId", event.getProductId());
            variables.put("reservationId", event.getReservationId());

            // inventory.reserved - это внутреннее событие, логируем без отправки email
            notificationService.logNotificationWithoutSending(
                    "inventory.reserved",
                    null,
                    variables
            );
    ...
    public void handleUserCreated(
        ...
        try {
            ...
            Map<String, String> variables = new HashMap<>();
            variables.put("userId", event.getUserId());
            variables.put("username", event.getUsername() != null ? event.getUsername() : "User");
            variables.put("email", event.getEmail());
            variables.put("firstName", event.getFirstName() != null ? event.getFirstName() : "");
            variables.put("lastName", event.getLastName() != null ? event.getLastName() : "");
    ...
    public void handleInventoryFailed(
        ...
        try {
            ...
            variables.put("orderId", event.getOrderId());
            variables.put("productId", event.getProductId());
            variables.put("reason", event.getReason() != null ? event.getReason() : "Insufficient stock");
            variables.put("requestedQuantity", String.valueOf(event.getRequestedQuantity()));

            // Отправляем уведомление только если есть email
            if (event.getUserEmail() != null && !event.getUserEmail().isEmpty()) {
                ...
                log.warn("⚠️ No user email in inventory.failed event, logging only for orderId: {}", event.getOrderId());
                notificationService.logNotificationWithoutSending(
                        "inventory.failed",
                        event.getUserId(),
                        variables
            ...
            log.info("✅ Successfully processed inventory.failed event for orderId: {}", event.getOrderId());

        } catch (Exception e) {
            log.error("❌ Error processing inventory.failed event", e);
        }
    }
