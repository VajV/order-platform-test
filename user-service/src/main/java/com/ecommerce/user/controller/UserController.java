...
public class UserController {
    ...
    @PreAuthorize("hasRole('ADMIN') or @userService.isCurrentUser(#id)")
    @SecurityRequirement(name = "Bearer Authentication")
    @Operation(summary = "Update user profile by ID (admin or owner)")
    ...
    @PreAuthorize("hasRole('ADMIN') or @userService.isCurrentUser(#id)")
    @SecurityRequirement(name = "Bearer Authentication")
    @Operation(summary = "Get user roles (admin or owner)")
    public ResponseEntity<java.util.List<String>> getUserRoles(@PathVariable Long id) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    @SecurityRequirement(name = "Bearer Authentication")
    @Operation(summary = "Search users by filters (admin/manager)")
    public ResponseEntity<Page<UserResponse>> searchUsers(
            @RequestParam(required = false) String username,
            @RequestParam(required = false) String email,
            @RequestParam(required = false) String role,
            Pageable pageable) {
        return ResponseEntity.ok(userService.searchUsers(username, email, role, pageable));
    }
